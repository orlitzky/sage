...
cdef class interval_bernstein_polynomial:
    ...
    def try_rand_split(self, context ctx, logging_note):
        ...
        EXAMPLES::
            ...
            sage: bp2
            <IBP: (-20, -2, 36, 101, 200) + [0 .. 6) over [325/512 .. 1]>
            sage: bp1, bp2, _ = bp.try_rand_split(mk_context(seed=42), None)
            sage: bp1
            <IBP: (50, 39, 17, -4, -18) + [0 .. 6) over [0 .. 185/512]>
            sage: bp2
            <IBP: (-18, -43, -43, 27, 200) + [0 .. 6) over [185/512 .. 1]>
            sage: bp = mk_ibpf([0.5, 0.2, -0.9, -0.7, 0.99], neg_err=-0.1, pos_err=0.01)
            sage: bp1, bp2, _ = bp.try_rand_split(mk_context(), None)
            sage: bp1  # rel tol
            <IBP: (0.5, 0.3095703125, -0.2032012939453125, -0.5012094490230083, -0.35743821566211403) + [-0.10000000000000069 .. 0.010000000000000677] over [0 .. 325/512]>
            sage: bp2  # rel tol
            <IBP: (-0.35743821566211403, -0.2747144598513841, -0.045731925964355435, 0.37275390625, 0.99) + [-0.10000000000000069 .. 0.010000000000000677] over [325/512 .. 1]>
        """
...
def real_roots(p, bounds=None, seed=None, skip_squarefree=False, do_logging=False, wordsize=32, retval='rational', strategy=None, max_diameter=None):
    ...
    EXAMPLES::
        ...
        [((-31735994437081219/2305843009213693952,
           46475703649376785/1152921504606846976),
          1),
         ((33229038299640304905560103675810783999/332306998946228968225951765070086144,
           66458076609369698769767275873590126019/664613997892457936451903530140172288),
          1),
         ((8307259577432348466051793049444835505/83076749736557242056487941267521536,
           4153629793760718712349430785706696763/41538374868278621028243970633760768),
          1)]
        sage: real_roots(x^5 * (x^2 - 9999)^2 - 1, seed=42)
        [((-551336364252941/36028797018963968, 522061863734815/18014398509481984), 1),
         ((1063329225238830829380464981087251127985/10633823966279326983230456482242756608,
           4253316902751651687274748537718738511395/42535295865117307932921825928971026432),
          1),
         ((2126658452273990028513818575544236255425/21267647932558653966460912964485513216,
           66458076689697449945834599653561570465/664613997892457936451903530140172288),
          1)]
        sage: real_roots(x^5 * (x^2 - 9999)^2 - 1, wordsize=64)
        [((-545220231983904529914624311/39614081257132168796771975168,
           798446509303202797121621005/19807040628566084398385987584),
          1),
         ((68053070437663325198921433551857863244025/680564733841876926926749214863536422912,
           136106140895989104571240600297625244134965/1361129467683753853853498429727072845824),
          1),
         ((17013267614581444843079791686441845222735/170141183460469231731687303715884105728,
           8506633817621949508238762440175681434825/85070591730234615865843651857942052864),
          1)]
        sage: real_roots(x)
        [((-119/1024, 393/2048), 1)]
        sage: real_roots(x * (x-1))
        [((-119/1024, 393/2048), 1), ((13/16, 37/32), 1)]
        sage: real_roots(x-1)
        [((905/1024, 2441/2048), 1)]
        sage: real_roots(x*(x-1)*(x-2), bounds=(0, 2))
        [((0, 0), 1), ((393/512, 1417/1024), 1), ((2, 2), 1)]
        sage: real_roots(x*(x-1)*(x-2), bounds=(0, 2), retval='algebraic_real')
        [(0, 1), (1, 1), (2, 1)]
        sage: v = 2^40
        sage: real_roots((x^2-1)^2 * (x^2 - (v+1)/v))
        [((-4074071952670824845964688014696366894296184735416998412065668861662213775592586334119882677/4074071952668972172536891376818756322102936787331872501272280898708762599526673412366794752,
           -2037035976335412422982344007348183312343746252037653834108562278918134776421611575515646901/2037035976334486086268445688409378161051468393665936250636140449354381299763336706183397376),
           1),
        ((-9007199254741367/9007199254740992,
          -35184372088831/35184372088832),
          2),
        ((4271974071841820164790043412339104081643028983253271320386031980356993618650595589126070857431937/4271974071841820164790043412339104229205409044713305539894083215644439451561281100045924173873152,
          8543948143683640329580086824678208586412932472658773832916655932718681612147083149319364813572319/8543948143683640329580086824678208458410818089426611079788166431288878903122562200091848347746304),
          2),
        ((248661618205006399289836914959495038713305949648857959919421835204559949848946319247853/248661618204893321077691124073410420050228075398673858720231988446579748506266687766528,
          242833611528326561806481362265131880475098063374483082051803625472723574132788519697/242833611528216133864932738352939863330300854881517440156476551217363035650651062272),
          1)]
        ...
        sage: v = 2^60
        sage: real_roots((x-1) * (x-(v+1)/v), retval='interval')
        [(1.000000000000000000?, 1), (1.00000000000000001?, 1)]
        sage: real_roots((x-1) * (x-2), strategy='warp')
        [((8181/8203, 16993/15775), 1), ((2203/1893, 1417/631), 1)]
        sage: real_roots((x+3)*(x+1)*x*(x-1)*(x-2), strategy='warp')
        [((-53/11, -21/11), 1),
         ((-73/55, -31/33), 1),
         ((0, 0), 1),
         ((8181/8203, 16993/15775), 1),
         ((2203/1893, 1417/631), 1)]
    ...
    Now we play with algebraic real coefficients. ::
        ...
        sage: real_roots(p)
        [((8181/8203, 16993/15775), 1),
         ((9443/6941, 19517/13251), 1),
         ((5037/3155, 1417/631), 1)]
        ...
        [(1.00?, 1), (1.1?, 1), (1.37?, 1), (1.4?, 1), (2.00?, 1), (2.1?, 1)]
